...
NUM_AUG="${2:-10}"
TEST_SUBSAMPLE_SIZE="${3:-$SUBSAMPLE_SIZE}"  # Default to same as train
OUTPUT_DIR="data/sudoku-test-${SUBSAMPLE_SIZE}-aug-${NUM_AUG}"

echo "=== Preparing Sudoku Dataset ==="
echo "Train subsample size: $SUBSAMPLE_SIZE"
echo "Test subsample size: $TEST_SUBSAMPLE_SIZE"
...
python build_sudoku_dataset.py \
    ...
    --test-subsample-size "$TEST_SUBSAMPLE_SIZE" \
    --num-aug "$NUM_AUG"
