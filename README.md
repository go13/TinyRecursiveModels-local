...
- CUDA 12.6+ (tested with 12.8 for RTX 5090 Blackwell support)

## Quick Start (Recommended)

This fork includes convenience scripts for easy local setup and training:
...
# For older GPUs (CUDA 12.6):
# uv pip install torch --index-url https://download.pytorch.org/whl/cu126

# 3. Install other dependencies
uv pip install adam-atan2-pytorch einops tqdm coolname pydantic argdantic wandb omegaconf hydra-core huggingface_hub packaging ninja wheel setuptools setuptools-scm pydantic-core numba

# 4. Prepare dataset
./run_prepare_data.sh 1000 100   # 1000 samples, 100 augmentations

# 5. Train
./run_train.sh data/sudoku-test-1000-aug-100 epochs=50000 eval_interval=5000
```
...
| Script | Description |
|--------|-------------|
| `run_prepare_data.sh [samples] [augs]` | Prepare Sudoku dataset |
| `run_train.sh [data_path] [args...]` | Run training (wandb disabled by default) |
| `run_eval.sh [checkpoint] [data_path]` | Evaluate a checkpoint |

**Environment variables:**
- `WANDB_MODE=online` - Enable wandb logging (disabled by default)
- `DISABLE_COMPILE=""` - Enable torch.compile (disabled by default, requires python3-dev)
...
pip install adam-atan2-pytorch  # pure Python optimizer (replaces adam-atan2)
wandb login YOUR-LOGIN # login if you want the logger to sync results to your Weights & Biases (https://wandb.ai/)
```

### Changes from Original

This fork includes the following modifications for improved local compatibility:
- Replaced `adam-atan2` (requires CUDA compilation) with `adam-atan2-pytorch` (pure Python)
- Added convenience shell scripts for dataset preparation, training, and evaluation
- Scripts disable wandb and torch.compile by default for easier local development
- Includes sample Sudoku test dataset in `data/`
